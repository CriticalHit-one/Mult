...
threads = {
    ...
    "donatello": None, "donatik": None, "bot_timers": None,
...
config = {
    ...
    "update_url": "",
    "tt_follow_voice_id": "", "tt_gift_voice_id": "", "tt_like_voice_id": "", "tt_share_voice_id": "", "yt_member_voice_id": "", "tw_sub_voice_id": "", "tw_follow_voice_id": "", "tw_raid_voice_id": "", "tw_bits_voice_id": "", "tw_points_voice_id": "", "tw_hype_voice_id": "", "kk_follow_voice_id": "", "kk_sub_voice_id": "", "donation_voice_id": "",
    "yt_member_audio_path": "", "yt_member_media_path": "",
    "yt_enable_sub_alerts": True, "yt_enable_like_alerts": True,
    "yt_stats_poll_seconds": 60, "yt_like_threshold": 50,
...
# просто тихо вимикається, а резервний варіант (таймер при кожному донаті
# Donatik, вже реалізований окремо) продовжує працювати як і раніше.
# ============================================================================
...
class SourceAudioDuckMonitor:
    ...
    def start(self):
        if self.is_running:
            return
        names_raw = (config.get("music_duck_source_names") or "").strip()
        if not names_raw:
            return
        ...
        try:
            ...
            if not found_any:
                print("[Music Duck] Жодне джерело не вдалося підключити - експериментальне відстеження звуку вимкнено, залишається таймер при донаті")
                self._cleanup()
                return
            ...
            print("[Music Duck] Залишається резервний варіант - приглушення за таймером при кожному донаті Donatik")
            self._cleanup()
...
def _looks_like_target(text, target_lang):
    ...
    if target_lang == 'uk':
        return detect_cyrillic_lang(text) == 'uk'
    if target_lang == 'en':
        # немає жодної кирилиці -> вважаємо, що це вже латиниця/англійська
...
def translate_text_to(text, target_lang='uk'):
    ...
    try:
        ...
        source_lang = detected if detected in ('uk', 'ru') else 'en'
    ...
    if translated is None:
        try:
            ...
            print("[Translator] Google помилка: {}".format(e))
...
def process_filters_and_tts(platform, username, display_name, text, is_mod, is_broadcaster, is_subscriber=False):
    ...
    if len(clean_text) > 120:
        clean_text = clean_text[:120].rstrip() + " і так далі"
...
# ============================================================================
# ПОТОКИ (Twitch, YouTube, Kick, TikTok, Donatello, Donatik, Bot)
# ============================================================================
...
def youtube_collect_stats_snapshot(api_key, video_ids):
    ...
    if channels:
        ...
        for item in (ch_data or {}).get('items', []) or []:
            ...
            stats = item.get('statistics', {}) or {}
            if not cid or stats.get('hiddenSubscriberCount'):
                continue
            try:
                ...
                if video_ids:
                    ...
                    if enable_likes:
                        ...
                        for vid, total_likes in likes.items():
                            ...
                            try:
                                text = template.format(likes=reached, total=total_likes)
                            except Exception:
...
def kick_moderation_action(action, **kwargs):
    ...
    if action in ('timeout', 'ban'):
        ...
        if result is None:
            error = '{} [broadcaster_user_id={}, token scope="{}"]'.format(error, broadcaster_id, kick_user_oauth_cache.get('scope', ''))
    ...
    if action == 'unban':
        ...
        if not user_id:
            return {'ok': False, 'error': 'Немає user_id'}
        url = 'https://api.kick.com/public/v1/moderation/bans?broadcaster_user_id={}&user_id={}'.format(broadcaster_id, user_id)
    ...
    if action == 'delete':
        ...
        url = 'https://api.kick.com/public/v1/chat/{}?broadcaster_user_id={}'.format(urllib.parse.quote(message_id), broadcaster_id)
        result, error = kick_api_request(url, method='DELETE')
        if result is None:
            error = '{} [broadcaster_user_id={}, token scope="{}"]'.format(error, broadcaster_id, kick_user_oauth_cache.get('scope', ''))
        return {'ok': result is not None, 'error': error}
...
def donatello_worker(session_id):
    global config
    if not config["donatello_token"]:
        return
    last_checked_time = time.time()
    while session_is_current(session_id):
        try:
            req = urllib.request.Request("https://donatello.to/api/v1/donations", headers={'X-Token': config["donatello_token"], 'User-Agent': 'Mozilla/5.0'})
            with urllib.request.urlopen(req, timeout=5) as r:
                data = json.loads(r.read().decode('utf-8'))
                for don in data.get("donations", []):
                    d_time = time.mktime(time.strptime(don["createdAt"][:19], "%Y-%m-%dT%H:%M:%S"))
                    if d_time > last_checked_time:
                        username = don.get("clientName", "Анонім")
                        amount = don["amount"]
                        msg = don.get("comment", " ")
                        add_to_buffer("alerts", username, msg, "Донат {} гривень від {}. {}".format(amount, username, msg), is_alert=True, subtitle="Donatello: {} UAH".format(amount), priority=PRIORITY_MAP["donation"], tts_engine_override='piper' if (config.get('donation_voice_id') or '').strip() else '', tts_voice_id_override=(config.get('donation_voice_id') or '').strip())
...
def donatik_worker(session_id):
    global config
    if not config["donatik_token"]:
        return
    seen_donation_ids = set()
    first_poll = True
    while session_is_current(session_id):
        try:
            # Ендпоінт підтверджено офіційною документацією donatik.io
            # (наданою користувачем, скріни офіційного Google-документа):
            # GET https://api.donatik.ua/donations?token=...&fromDate=...&perPage=...
            # (без /v1/ - цей префікс був помилковим і робив запит мовчки
            # нерезультативним). Токен передається як query-параметр
            # ?token=, саме так показано в офіційному прикладі для цього
            # ендпоінту (для інших ендпоінтів у документації показаний
            # заголовок X-Token - додаємо обидва варіанти для надійності).
            today_str = time.strftime("%Y-%m-%d", time.gmtime())
            url = "https://api.donatik.ua/donations?{}".format(urllib.parse.urlencode({
                "token": config["donatik_token"],
                "fromDate": today_str,
                "perPage": 500,
            }))
            req = urllib.request.Request(url, headers={'X-Token': config["donatik_token"], 'User-Agent': 'Mozilla/5.0'})
            with urllib.request.urlopen(req, timeout=5) as r:
                raw = json.loads(r.read().decode('utf-8'))
                # Точна структура відповіді (обгортка на кшталт {"data":[...]}
                # чи просто список) не задокументована у наданих скрінах -
                # обробляємо обидва варіанти захисно.
                if isinstance(raw, list):
                    donations = raw
                else:
                    donations = raw.get("data") or raw.get("donations") or raw.get("items") or []

                # ID трактуємо як рядок і зберігаємо в set() бачених ID -
                # це коректно незалежно від того, число це чи UUID-рядок
                # (GraphQL-схема ID! у документації натякає на рядковий
                # тип), на відміну від порівняння "більше за попередній".
                current_ids = []
                for don in donations:
                    don_id = pick_first_non_empty(don.get("id"), don.get("paymentId"), None)
                    if don_id is None:
                        continue
                    current_ids.append((str(don_id), don))

                if first_poll:
                    # При першому запуску не озвучуємо всю історію за
                    # сьогодні - лише запам'ятовуємо, що вже було, і
                    # реагуємо на донати, які з'являться після цього.
                    seen_donation_ids = {did for did, _ in current_ids}
                    first_poll = False
                else:
                    for don_id, don in current_ids:
                        if don_id in seen_donation_ids:
                            continue
                        seen_donation_ids.add(don_id)
                        if len(seen_donation_ids) > MAX_CACHE_SIZE:
                            seen_donation_ids = {did for did, _ in current_ids}
                        username = don.get("username") or don.get("name") or don.get("donorName") or "Анонім"
                        amount = don.get("amount", 0)
                        msg = don.get("message") or don.get("comment") or " "
                        add_to_buffer("alerts", username, msg, "Донат {} гривень від {}. {}".format(amount, username, msg), is_alert=True, subtitle="Donatik: {} UAH".format(amount), priority=PRIORITY_MAP["donation"], tts_engine_override='piper' if (config.get('donation_voice_id') or '').strip() else '', tts_voice_id_override=(config.get('donation_voice_id') or '').strip())
                        # Donatik показує алерт і, за наявності замовленого
                        # кліпа, програє його через СВІЙ ВЛАСНИЙ окремий
                        # віджет (widget.donatik.ua/...), який працює поза
                        # цим скриптом. Офіційна документація не містить
                        # жодного поля для кліпу/медіа в донаті - тож
                        # дізнатись реальну тривалість кліпа з публічного
                        # API неможливо. Приглушуємо музику на фіксований,
                        # налаштовуваний час "про запас" при КОЖНОМУ донаті
                        # - єдиний чесний варіант за відсутності цих даних.
                        if config.get("music_duck_on_alert", True):
                            duck_seconds = max(1, int(config.get("donatik_clip_duck_seconds", 20) or 20))
                            audio_ducking_begin()
                            timer = threading.Timer(duck_seconds, audio_ducking_end)
                            timer.daemon = True
                            timer.start()
        except:
            pass
...
body{background:#0e0e13;color:#f1f1f7;font-family:'Segoe UI',system-ui,sans-serif;margin:0;padding:0;font-size:14px}
.topbar{position:sticky;top:0;z-index:50;background:rgba(14,14,19,0.95);backdrop-filter:blur(10px);border-bottom:1px solid rgba(255,255,255,0.1);padding:12px 14px;display:flex;flex-direction:column;gap:8px}
.topbar-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.title{font-weight:bold;font-size:16px;color:#ffb703;flex:1}
.search-input{flex:1;min-width:120px;padding:8px 12px;border-radius:20px;border:1px solid rgba(255,255,255,0.2);background:rgba(0,0,0,0.4);color:#f1f1f7;font-size:13px}
.btn{background:linear-gradient(135deg,#00f2fe 0%,#4facfe 100%);padding:9px 18px;border-radius:20px;font-weight:bold;border:none;color:#08131c;cursor:pointer;font-size:13px;white-space:nowrap}
.btn:disabled{opacity:0.5;cursor:default}
.btn-secondary{background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);padding:9px 16px;border-radius:20px;color:#e2e2e9;cursor:pointer;font-size:13px}
.status{font-size:12px;color:#8a8a99;min-height:16px}
.status.ok{color:#4caf50}
.status.err{color:#ff416c}
.container{padding:12px;display:flex;flex-direction:column;gap:10px}
.section{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:14px;overflow:hidden}
.section-header{padding:12px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;user-select:none;border-left:4px solid var(--accent,#4facfe)}
.section-header:hover{background:rgba(255,255,255,0.03)}
.section-title{font-weight:bold;flex:1}
.section-caret{transition:transform 0.2s;opacity:0.6}
.section.open .section-caret{transform:rotate(90deg)}
.section-body{display:none;padding:4px 14px 14px 18px;border-top:1px solid rgba(255,255,255,0.06)}
.section.open .section-body{display:flex;flex-direction:column;gap:12px}
.field{display:flex;flex-direction:column;gap:5px}
.field.hidden{display:none}
.field-label{font-size:12.5px;color:#c8c8d4}
input[type=text],input[type=number],input[type=password],textarea{width:100%;padding:9px 11px;border-radius:10px;border:1px solid rgba(255,255,255,0.15);background:rgba(0,0,0,0.35);color:#f1f1f7;font-size:13.5px;font-family:inherit}
...
.btn-clear-path{flex:0 0 auto;width:32px;padding:0;cursor:pointer;border-radius:6px;border:1px solid #3a4152;background:#242a38;color:#e6e9f0;font-size:13px;line-height:30px}
.btn-clear-path:hover{background:#3a2530;border-color:#7a3040;color:#ff8a9c}
.secret-wrap input{padding-right:38px}
.eye-btn{position:absolute;right:4px;top:50%;transform:translateY(-50%);background:none;border:none;color:#8a8a99;cursor:pointer;font-size:15px;padding:4px 8px}
.switch{position:relative;display:inline-block;width:42px;height:24px;flex-shrink:0}
.switch input{opacity:0;width:0;height:0}
.slider{position:absolute;cursor:pointer;top:0;left:0;right:0;bottom:0;background:rgba(255,255,255,0.15);border-radius:24px;transition:0.2s}
.slider:before{position:absolute;content:"";height:18px;width:18px;left:3px;bottom:3px;background:#e2e2e9;border-radius:50%;transition:0.2s}
input:checked + .slider{background:linear-gradient(135deg,#00f2fe 0%,#4facfe 100%)}
...
.platform-chip{display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:16px;padding:6px 12px;font-size:12.5px}
.hint{font-size:11px;color:#6f6f7c;line-height:1.4}
.empty-state{padding:30px;text-align:center;color:#6f6f7c;font-size:13px}
.fatal-error{display:none;margin:12px;padding:14px;border-radius:12px;background:rgba(255,65,108,0.12);border:1px solid #ff416c;color:#ffb3c1;font-size:12.5px;white-space:pre-wrap;font-family:monospace}
.fs-modal-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:100;align-items:center;justify-content:center}
.fs-modal-overlay.open{display:flex}
.fs-modal{background:#15151c;border:1px solid rgba(255,255,255,0.15);border-radius:14px;width:92%;max-width:520px;max-height:80vh;display:flex;flex-direction:column;overflow:hidden}
...
.fs-entry{display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:8px;cursor:pointer;font-size:13px}
.fs-entry:hover{background:rgba(255,255,255,0.07)}
...
body.netmon-active .chat-container{bottom:32px}
...
<style>
*{box-sizing:border-box}
body{background:#0e0e13;margin:0;padding:16px;font-family:'Segoe UI',system-ui,sans-serif;color:#e2e2e9;height:100vh;overflow-y:auto}
h1{font-size:1.1em;margin:0 0 4px;color:#ff0050}
...
.top-card{background:rgba(255,255,255,0.05);border-radius:12px;padding:12px;border:1px solid rgba(255,255,255,0.1)}
.top-card h3{margin:0 0 8px;font-size:0.9em;color:#00f2fe}
.top-row{display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:0.85em}
.top-row:last-child{border-bottom:none}
.top-row img{width:24px;height:24px;border-radius:50%;object-fit:cover;background:#333;flex-shrink:0}
...
.top-row .nick a{color:#e2e2e9;text-decoration:none}
...
.roster-item{display:flex;align-items:center;gap:10px;background:rgba(255,255,255,0.03);border-radius:10px;padding:8px 12px}
.roster-item img{width:36px;height:36px;border-radius:50%;object-fit:cover;background:#333;flex-shrink:0}
.roster-item .name{font-weight:600;flex-shrink:0;min-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.roster-item .name a{color:#e2e2e9;text-decoration:none}
...
.icon-btn{background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.12);color:#e2e2e9;border-radius:8px;padding:5px 9px;font-size:0.9em;cursor:pointer;line-height:1}
.icon-btn:hover{background:rgba(255,0,80,0.25);border-color:#ff0050}
.icon-btn:disabled{opacity:0.3;cursor:default}
.day-selector{display:flex;align-items:center;gap:8px;margin-bottom:14px;flex-wrap:wrap}
.danger-btn{background:rgba(255,0,80,.18);border:1px solid rgba(255,0,80,.5);color:#ff8fae;border-radius:8px;padding:6px 12px;font-size:0.82em;cursor:pointer}
.danger-btn:hover{background:rgba(255,0,80,.32);color:#fff}
.day-selector label{font-size:0.85em;color:#9a9aa5}
.day-selector select{background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.15);color:#e2e2e9;border-radius:8px;padding:6px 10px;font-size:0.85em}
...
.modal{background:#16161d;border:1px solid rgba(255,255,255,0.15);border-radius:14px;width:100%;max-width:560px;max-height:80vh;display:flex;flex-direction:column}
...
.modal-close{background:none;border:none;color:#9a9aa5;font-size:1.3em;cursor:pointer}
.modal-body{padding:12px 16px;overflow-y:auto}
.log-row{padding:7px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:0.87em;display:flex;gap:10px}
...
<div class="top-grid">
<div class="top-card"><h3 id="topAllTitle">🌐 Топ глядачів усіх мереж</h3><div id="topAll"></div></div>
...
def synthesize_google_tts_bytes(text, lang, speed=1.0):
    g_url = "https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl={}&q={}".format(lang, urllib.parse.quote(text))
...
def handle_kick_webhook_event(event_name, payload):
    ...
    if event_name in ('channel.followed', 'channel.follow', 'follow'):
        ...
        return True
    if event_name in ('channel.subscription.created', 'channel.subscription.renewal', 'channel.subscription', 'subscription', 'subscription.renewal'):
        ...
        return True
    if event_name in ('kicks.gifted', 'channel.subscription.gifted', 'subscription.gifted'):
        if not config.get('kk_enable_subs', True):
            return False
...
def sn_active_platforms():
    ...
    for slug, label in SN_PLATFORM_DEFS:
        ...
        if url:
            ...
            if slug in SN_PLATFORM_STYLES:
                item["style"] = SN_PLATFORM_STYLES[slug]
            result.append(item)
    return result
...
def sn_dc_send_post(img_path, title_text, desc_text, fields, mention):
    ...
    payload = {
        ...
        "embeds": [{
            ...
            "color": 16711680,
            "image": {"url": "attachment://preview.jpg"},
            "fields": fields,
        }],
    }
    if img_path and os.path.isfile(img_path):
        http_post_multipart(webhook, {"payload_json": json.dumps(payload)}, file_field_name="files[0]", file_path=img_path, file_name_override="preview.jpg")
    else:
        http_post_json(webhook, payload)
    log_status("StreamNotify", "Discord post done")
        ...
        try:
            ...
            if path == '/callback/twitch':
                ...
                params = {
                    ...
                    'scope': ' '.join(['user:read', 'channel:read', 'events:subscribe', 'moderation:ban', 'moderation:chat_message:manage']),
                    'state': state,
                ...
                self.end_headers()
                if ok:
                    self.wfile.write('<html><body><h2>Kick OAuth успішно завершено</h2><p>Access token отримано. Можна закривати цю вкладку.</p></body></html>'.encode('utf-8'))
                else:
...
PLATFORM_WORKERS = {
    ...
    "donatello": donatello_worker,
    "donatik": donatik_worker,
    "bot_timers": bot_timer_worker,
}
PLATFORM_START_ORDER = ["twitch", "twitch_eventsub", "kick", "youtube",
                        "youtube_stats", "tiktok", "donatello", "donatik",
                        "bot_timers"]
...
def stop_platform(name, reason='stop', join_timeout=5.0):
...
def stop_all_platforms(reason='kill_services'):
...
def kill_services():
    ...
    global_session_id += 1
    stop_all_platforms(reason='kill_services')
    async_runtime.stop()
...
def script_description():
    upd_html = ""
    upd_url = (config.get("update_url") or "").strip()
    if upd_url:
        if not upd_url.lower().startswith(("http://", "https://")):
            upd_url = "https://" + upd_url
...
def script_properties():
    ...
    obs.obs_property_set_long_description(
        ...
        "Донати\n"
        "- Токени Donatello / DonatIK — з особистих кабінетів цих сервісів.\n\n"
