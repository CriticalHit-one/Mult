...
def generate_top_likes_widget_html():
    """Автономний OBS Browser Source віджет: топ-10 глядачів TikTok за лайками.
...
body{font-family:'Segoe UI',sans-serif;background:transparent;color:#fff;overflow:hidden}
.wrap{width:340px;padding:14px 16px;background:linear-gradient(160deg,rgba(20,20,28,0.85),rgba(35,15,45,0.85));border-radius:16px;box-shadow:0 8px 24px rgba(0,0,0,0.35)}
...
.row{display:flex;align-items:center;gap:10px;padding:6px 8px;border-radius:10px;margin-bottom:4px;background:rgba(255,255,255,0.06);animation:fadeIn .3s ease}
...
async function update(){
  try{
    ...
    const data=await res.json();
    const items=(data&&data.top_likes)||[];
    const list=document.getElementById('list');
    if(!items.length){list.innerHTML='<div class="empty">Поки що немає даних про лайки</div>';return;}
    list.innerHTML=items.slice(0,LIMIT).map((u,i)=>{
      const place=i+1;
      const medal=place===1?'🥇':place===2?'🥈':place===3?'🥉':(place+'.');
      const rowClass='row'+(place<=3?(' r'+place):'');
      const name=(u.nickname||u.display_name||u.username||'Глядач');
      const avatar=u.avatar_url?`<img class="avatar" src="${u.avatar_url}">`:'<div class="avatar"></div>';
      return `<div class="${rowClass}"><span class="place">${medal}</span>${avatar}<span class="name">${name}</span><span class="likes">❤️ ${fmt(u.count)}</span></div>`;
    }).join('');
